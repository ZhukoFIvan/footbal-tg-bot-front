...
import { BadgeCreateInput } from '@/app/components/admin/forms/schemas/validationSchemas'

export default function CreateBadgePage() {
	const router = useRouter()
	const [createBadge, { isLoading: creating }] = useCreateBadgeMutation()

	const handleSubmit = async (data: BadgeCreateInput) => {
		try {
	...
	return (
		<div className='min-h-screen bg-background pb-24'>
			...
			<div className='sticky top-0 bg-background/80 backdrop-blur-xl z-40 border-b border-white/5'>
				<div className='container mx-auto px-4 py-4'>
					<div className='flex items-center gap-3'>
						...
						<h1 className='text-2xl font-bold text-foreground'>Создать бейдж</h1>
					</div>
				</div>
			</div>
			...
			<div className='container mx-auto px-4 py-6 max-w-2xl'>
				<BadgeForm mode='create' onSubmit={handleSubmit} isSubmitting={creating} submitText='Создать' />
			</div>
		</div>
	)
}
