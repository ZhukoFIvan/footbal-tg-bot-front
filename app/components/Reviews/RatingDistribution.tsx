...
import { ProductRating } from '@/app/store/api/reviewsApi'
		...
		<div className="space-y-4">
			...
			<div className="flex items-center gap-4">
				...
				<div className="flex-1 space-y-2">
					{[5, 4, 3, 2, 1].map((star) => {
						const count = rating_distribution[star.toString() as keyof typeof rating_distribution]
						const percentage = getPercentage(count)

						return (
							<div key={star} className="flex items-center gap-2">
								<span className="text-sm text-foreground/70 w-3">
									{star}
								</span>
								<svg
